    ...
    if (dataElement && (Object.getOwnPropertyNames(dataElement).length > 0 || params.includeEmptyRows)) {
      ...
      params.fields.forEach(function (fieldElement) {
        ...
        if (val !== undefined) {
          ...
          //Workaround: If a backslash comes at the end of the string, JSON.stringify('\') results in `"\\"`
          //which ultimately merges the next value into this one,
          //adding space after backslash would not.
          stringifiedElement = stringifiedElement.replace(/\\\"$/,'\"');
